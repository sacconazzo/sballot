...
export default function Stats(props) {
  ...
  const updateAccounts = async () => {
    try {
      ...
      if (newStats[2].val / newStats[0].val >= 0.1)
        newStats[1].perc = `${Math.round((newStats[1].val / newStats[0].val) * 100, 2)}%`
      if (newStats[2].val / newStats[0].val >= 0.1)
        newStats[2].perc = `${Math.round((newStats[2].val / newStats[1].val) * 100, 2)}%`
